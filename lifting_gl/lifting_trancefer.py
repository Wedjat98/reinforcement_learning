...
for n_repeat in range(1,n_times + 1):
    ...
    path_target = 'C:\\Users\\kiku\\PycharmProjects\\グループスタディ\\lifting_gl\\reward_trancefer_{}.csv'.format(n_repeat)

    gamma = 0.99    #割引率
    alpha = 0.5     #学習率
    n_episodes = 500    #総試行回数(最大episode)

    q_func = QFunction(env.observation_space.shape[0], env.action_space.n)
    serializers.load_npz(path_source, q_func)
    optimizer = chainer.optimizers.Adam(eps=1e-2)
    optimizer.setup(q_func)
    explorer = chainerrl.explorers.LinearDecayEpsilonGreedy(start_epsilon=0.5, end_epsilon=0.1, decay_steps=n_episodes, random_action_func=env.action_space.sample)
