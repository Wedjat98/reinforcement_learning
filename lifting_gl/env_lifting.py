...
class LiftingEnv(gym.Env):
    ...
    def __init__(self):
        ...
        #各センサーの最大値
        high = np.array([
            self.cx_threshold,          #ラケットの横座標
            np.finfo(np.float32).max,   #ラケットの横速度
            self.bx_threshold,          #ボールの横座標
            self.by_threshold,          #ボールの縦座標
            np.finfo(np.float32).max    #ボールの横速度
            ])
    ...
    def _reset(self):
        ...
        sensor = np.array([0, 0, 0, self.ballPosition])
