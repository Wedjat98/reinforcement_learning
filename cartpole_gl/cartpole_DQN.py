...
for n_repeat in range(1, n_times+1):
    path_source = 'C:\\Users\\kiku\\PycharmProjects\\グループスタディ\\cartpole_gl' \
                  '\\source_model_{}.npz'.format(n_repeat)
    path_target = 'C:\\Users\\kiku\\PycharmProjects\\グループスタディ\\cartpole_gl' \
                  '\\reward_DQN_{}.csv'.format(n_repeat)
    ...
    num_episodes = 500  #総試行回数(最大episode数)
    ...
    with open(path_target, 'w') as f:   #結果ファイルの作成
        for episode in range(1, num_episodes+1):  #試行数分繰り返す(episode)
            writer = csv.writer(f, lineterminator='\n')
            observation = env.reset()
            done = False
            reward = 0
            R = 0
            for t in range(max_number_of_steps):  #1試行のループ(step)
                if episode % 100 == 0:
                    env.render()    #画面表示
                ...
                if done:
                    break
            writer.writerow([episode, R, agent.get_statistics()])     #結果(報酬値)の書き込み
            agent.stop_episode_and_train(observation, reward, done)
            if episode == 1 or episode % 10 == 0:
                print('NO.', n_repeat, 'episode:', episode, 'R:', R, 'statistics:', agent.get_statistics())   #結果の表示
        serializers.save_npz(path_source, agent.target_model)   #学習知識の保存
